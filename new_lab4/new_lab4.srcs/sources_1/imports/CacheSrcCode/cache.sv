...
localparam WAY_LEN = 1 + $clog2(WAY_CNT); // 路编号长度
...
reg [TAG_ADDR_LEN-1:0] cache_tags   [SET_SIZE][WAY_CNT];            // SET_SIZE*WAY_CNT个setTAG
reg                    valid        [SET_SIZE][WAY_CNT];            // SET_SIZE*WAY_CNT个valid(有效位)
reg                    dirty        [SET_SIZE][WAY_CNT];            // SET_SIZE*WAY_CNT个dirty(脏位)
reg [       WAY_LEN:0] next_way     [SET_SIZE];                     // SET_SIZE个next_way(下次换入使用的路)
...
always @ (*) begin              // 判断 输入的address 是否在 cache 中命中
    ...
    for (integer j=0; j<WAY_CNT; j++) begin
        if (valid[set_addr][j] && cache_tags[set_addr][j] == tag_addr) begin // 如果 cache line有效，并且tag与输入地址中的tag相等，则命中
            cache_hit = 1'b1;
            way = j;
        end
    end
